...
void generate_report(int shutdown)
{
    ...
    sprintf(report_str, "%s%s\n", time_s, temp_s);
    SSL_write(sslClient, report_str, strlen(report_str));
    if (lflag)
        fprintf(logfile, report_str);
}
...
int main(int argc, char *argv[])
{
    ...
    char id_str[12];
    sprintf(id_str, "ID=%s\n", id);
    SSL_write(sslClient, id_str, strlen(id_str));
    ...
    while (1)
    {
        ...
        while ((int) difftime(curr, prev) < period)
        {
            ...
            if (pfd.revents & POLLIN) {
                SSL_read(sslClient, buffer, BUFFER_SIZE);
